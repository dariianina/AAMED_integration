...
void FLED::drawEllipses()
{
    ...
    //cv::imwrite("Output Data/Ellipses.png", Img_T);
...
//	Mat Img_T = ImgG;
	...
	//cv::imshow("Ellipses", Img_T);
        //cv::imwrite("Ellipses.png", Img_T);
	if(savepath.length() > 0)
		imwrite(savepath, Img_T);
}

void FLED::drawFLED(Mat ImgG, Mat fullImg, string savepath)
{
	char arcnum[128];
	Mat Img_T = fullImg.clone();
	//cvtColor(Img_T, Img_T, CV_GRAY2BGR);
//	Mat Img_T = ImgG;
	cv::RotatedRect temp;
	//for (int i = 0; i < FSA_Lines.size(); i++)
	...
	for (int i = 0; i < detEllipses.size(); i++)
	{
		//if (detEllipseScore[i] < 0.7)
		//	continue;
		temp.center.x = int(detEllipses[i].center.y / ImgG.cols * Img_T.cols);
		temp.center.y = int(detEllipses[i].center.x / ImgG.rows * Img_T.rows);
		temp.size.height = int(detEllipses[i].size.width / ImgG.rows * Img_T.rows);
		temp.size.width = int(detEllipses[i].size.height / ImgG.cols * Img_T.cols);
		if(temp.size.width < 50)
		{
			continue;
		}
		temp.angle = -detEllipses[i].angle;
...
#endif
		//sprintf_s(arcnum, "%.2f", detEllipseScore[i]);
		cv::putText(Img_T, string(arcnum) + " " + std::to_string(temp.size.width), temp.center, cv::FONT_HERSHEY_SIMPLEX, 0.5, cv::Scalar(0, 0, 255), 1);
		//cout << "The " << i << "th ellipse's score is :" << detEllipseScore[i] << endl;
	}
	//cv::imshow("Ellipses", Img_T);
        //cv::imwrite("Ellipses.png", Img_T);
...
float** FLED::getEllipses(int& count)
{
	cv::RotatedRect temp;

	int ell_count = 15;
	int ell_coords = 6; // 5 for coords and 1 for score
	float** ellipse_coords = new float*[ell_count];
	for (int i = 0; i < ell_count; ++i)
	{
		ellipse_coords[i] = new float[ell_coords];
	}
	...
	for (int i = 0; i < detEllipses.size(); i++)
	{
		...
		if(found_count < ell_count)
		{
			ellipse_coords[found_count][0] = temp.center.x;
			ellipse_coords[found_count][1] = temp.center.y;
			ellipse_coords[found_count][2] = temp.size.height;
			ellipse_coords[found_count][3] = temp.size.width;
			ellipse_coords[found_count][4] = temp.angle;
			ellipse_coords[found_count][5] = detEllipseScore[i];
			found_count += 1;
		}
		else break;
